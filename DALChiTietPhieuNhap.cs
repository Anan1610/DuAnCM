...
namespace DAL_QLVT
{
    public class DALChiTietPhieuNhap
    {
        ...
        public void Search(string keyword, out List<ChiTietPhieuNhap> results)
        {
            string sql = "SELECT * FROM ChiTietPhieuNhap WHERE ChiTietNhapID LIKE @0 OR PhieuNhapID LIKE @0 OR VatTuID LIKE @0";
            List<object> args = new List<object> { $"%{keyword}%" };
            results = SelectBySql(sql, args);
        }
    }
}
